...
- **지도**: 실제 네이버 지도(Dynamic Map) 위에 전시 위치를 건대입구 기준 이동시간과 함께 마커로 표시. 마커·하단 카드를 탭하면 바로 상세 바텀시트가 열림(하단 캐러셀과도 연동), "네이버지도로 길찾기" 딥링크 제공. 📍 "내 위치" 버튼으로 브라우저 위치 권한을 허용하면 직선거리 기반 대략적 이동시간을 추가로 보여줌(정확한 경로 계산은 아님). 수도권 밖 전시는 지도에 안 찍고 상단 칩으로만 안내
- **저장**: 관심 전시 북마크, 종료 D-7 임박 시 탭 진입 시 인앱 배너로 안내(실제 백그라운드 푸시 알림은 아님 — 앱을 열어야 표시됨)
- **더보기**: 기준 위치/알림/데이터 출처 안내

전시 카드를 탭하면 상세 바텀시트가 열리고 작가·주소·전시시간·**입장료**(정보가 있는 경우만 표시)·전시특징·작가소개·"영상에서 꼭 봐야 할 내용"·추천 영상 링크를 확인할 수 있습니다.
